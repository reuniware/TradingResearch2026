...
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
#property copyright "VPIN Trading Bot - Version FTMO Optimized"
...
//--- Paramètres VPIN
input double   InpBucketSize      = 80.0;     // Taille du bucket (réduit pour plus de signaux)
input int      InpVpinWindow      = 20;       // Période VPIN (Réactivité)
input int      InpSigmaPeriod     = 100;      // Période volatilité
input double   InpTradeThreshold  = 0.75;     // Seuil VPIN Moyen (augmenté pour plus de fiabilité)
input double   InpInstantThreshold= 0.90;     // Seuil VPIN Instantané (augmenté)
input bool     InpEnableLogging   = true;     // Log CSV

//--- Paramètres de Trading FTMO
input group "FTMO Trading Settings"
input double   InpMaxLotSize      = 0.03;     // Taille max du lot (sécurisé pour 10k€)
input double   InpRiskPerTrade    = 1.0;      // Risque par trade en % du capital
input int      InpStopLoss        = 2000;     // Stop Loss en points (20 USD pour XAUUSD)
input int      InpTakeProfit      = 4000;     // Take Profit en points (40 USD, ratio 1:2)
input int      InpMagicNumber     = 123456;   // Magic Number
input bool     InpCloseOpposite   = false;    // Fermer la position opposée (désactivé pour FTMO)

//--- Paramètres de Sécurité FTMO
input group "FTMO Risk Management"
input int      InpMaxTradesPerDay = 8;        // Maximum de trades par jour
input double   InpMaxDailyDrawdown = 3.0;     // Drawdown max journalier en % (stop auto)
input int      InpMinMinutesBetweenTrades = 2; // Minutes minimum entre deux trades
input double   InpMaxSpreadPoints = 30;       // Spread maximum en points (0.30 USD pour XAUUSD)
input bool     InpUseTimeFilter   = true;     // Filtrer les horaires à risque
input string   InpRiskHours       = "14:00-16:00"; // Heures à éviter (news US)
...
bool CheckFTMORisk()
{
   // 1. Vérifier si le trading est bloqué
   if(g_isTradingBlocked)
   {
      Comment("TRADING BLOQUE: ", g_blockReason);
      return false;
   }
   
   // 2. Réinitialiser le compteur journalier
   ...
   if(today.day != lastReset.day || today.mon != lastReset.mon || today.year != lastReset.year)
   {
      ...
      g_lastResetDate = currentTime;
      g_isTradingBlocked = false;
   }
   
   // 3. Vérifier le nombre max de trades par jour
   if(g_tradesToday >= InpMaxTradesPerDay)
   {
      g_isTradingBlocked = true;
      g_blockReason = "Maximum trades journalier atteint (" + IntegerToString(InpMaxTradesPerDay) + ")";
      return false;
   }
   
   // 4. Vérifier le drawdown journalier
   double currentEquity = AccountInfoDouble(ACCOUNT_EQUITY);
   double dailyDrawdown = (g_dailyStartBalance - currentEquity) / g_dailyStartBalance * 100.0;
   
   if(dailyDrawdown > InpMaxDailyDrawdown)
   {
      g_isTradingBlocked = true;
      g_blockReason = "Drawdown journalier maximum atteint (" + DoubleToString(dailyDrawdown, 1) + "%)";
      return false;
   }
   
   // 5. Vérifier le délai minimum entre trades
   if(g_lastTradeTime > 0 && (currentTime - g_lastTradeTime) < (InpMinMinutesBetweenTrades * 60))
   {
      int minutesLeft = InpMinMinutesBetweenTrades - (int)((currentTime - g_lastTradeTime) / 60);
      Comment("Attente obligatoire: ", minutesLeft, " minutes avant prochain trade");
      return false;
   }
   
   // 6. Vérifier le spread
   double spread = SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);
   double maxSpread = InpMaxSpreadPoints;
   if(spread > maxSpread)
   {
      Comment("Spread trop élevé: ", spread, " points (max: ", maxSpread, ")");
      return false;
   }
   
   // 7. Filtrer les heures à risque
   if(InpUseTimeFilter && IsRiskHour())
   {
      Comment("Trading bloqué: Heure à risque (news US)");
      return false;
   }
...
bool IsRiskHour()
{
   ...
   // Parser la plage horaire InpRiskHours (format "HH:MM-HH:MM")
   string riskHours = InpRiskHours;
   int dashPos = StringFind(riskHours, "-");
   if(dashPos == -1) return false;
   
   string startTime = StringSubstr(riskHours, 0, dashPos);
   string endTime = StringSubstr(riskHours, dashPos + 1);
...
//+------------------------------------------------------------------+
//| Calcul de lot dynamique basé sur le risque                       |
//+------------------------------------------------------------------+
double CalculateLotSize()
{
   ...
   // Limiter au lot maximum configuré
   double maxLot = InpMaxLotSize;
   ...
   Print("Lot calculé: ", DoubleToString(calculatedLot, 2), " (Risque: ", InpRiskPerTrade, "% du capital)");
...
//+------------------------------------------------------------------+
//| Gestion des Ordres (Version FTMO Optimisée)                      |
//+------------------------------------------------------------------+
void ManagePosition(string direction)
{
   if(direction != "ACHAT (Hausse)" && direction != "VENTE (Baisse)") return;
   
   // Vérifier les risques FTMO avant d'ouvrir une position
   if(!CheckFTMORisk())
   {
      Print("Trade bloqué par gestion des risques: ", g_blockReason);
      return;
   }
   ...
   if(direction == "ACHAT (Hausse)")
   {
      ...
      if(trade.Buy(lotSize, _Symbol, ask, sl, tp, "VPIN Buy Signal"))
      {
         g_lastTradeTime = TimeCurrent();
         g_tradesToday++;
         Print("Ordre ACHAT exécuté. Lot: ", DoubleToString(lotSize, 2), " VPIN: ", DoubleToString(g_lastVpin, 4));
         LogAlert("TRADE_BUY", g_lastVpin, direction, g_lastNetFlow, ask);
      }
      else
      {
         Print("Échec ordre ACHAT. Erreur: ", GetLastError());
      }
   }
   else if(direction == "VENTE (Baisse)")
   {
      ...
      if(trade.Sell(lotSize, _Symbol, bid, sl, tp, "VPIN Sell Signal"))
      {
         g_lastTradeTime = TimeCurrent();
         g_tradesToday++;
         Print("Ordre VENTE exécuté. Lot: ", DoubleToString(lotSize, 2), " VPIN: ", DoubleToString(g_lastVpin, 4));
         LogAlert("TRADE_SELL", g_lastVpin, direction, g_lastNetFlow, bid);
      }
      else
      {
         Print("Échec ordre VENTE. Erreur: ", GetLastError());
      }
   }
}
...
int OnInit()
{
   ...
   Print("=== FTMO RISK MANAGEMENT ACTIVATED ===");
   ...
   Print("Min Minutes Between Trades: ", InpMinMinutesBetweenTrades);
   Print("Max Spread: ", InpMaxSpreadPoints, " points");
   Print("Risk Hours: ", InpRiskHours);
   Print("======================================");
...
void OnTick()
{
   ...
   string riskStatus = "OK";
   if(g_isTradingBlocked) riskStatus = "BLOQUE: " + g_blockReason;
   
   Comment("=== VPIN TRADING BOT (FTMO v9.10) ===",
           ...
           "\nTrades Aujourd'hui: ", g_tradesToday, "/", InpMaxTradesPerDay,
           "\nDrawdown Journalier: ", DoubleToString(dailyDrawdown, 2), "% / ", InpMaxDailyDrawdown, "%",
           "\nSpread: ", DoubleToString(currentSpread, 1), " / ", InpMaxSpreadPoints, " pts",
           "\nStatut: ", riskStatus);
}
...
void CalculateVpin()
{
   ...
   // --- LOGIQUE DE TRADING AVEC FILTRES FTMO ---
   ...
   if(vpin > InpTradeThreshold)
   {
      ...
      // Déclenchement Trade (avec vérifications FTMO)
