...
// Structure pour mémoriser l'état des alertes par symbole ET par timeframe (Séparation SSB et Kijun)
struct SymbolState {
   string   name;
   ENUM_TIMEFRAMES tf;
...
void ScanSymbol(string symbol, ENUM_TIMEFRAMES tf)
{
   ...
   if(close_0 <= 0 || ssb_0 <= 0 || kijun_0 <= 0) 
   {
      IndicatorRelease(handle);
      return;
   }
   ...
   // Le prix s'approche de la SSB : avant il était au-delà de la tolérance, maintenant il est dedans
   ...
   if(is_approaching_ssb)
   {
      if(!AlreadyAlerted(symbol, time_buffer[0], tf, "SSB")) 
      {
         ...
         string msg = StringFormat("APPROCHE SSB | %s | %s | Prix: %.5f | SSB: %.5f | %s | Distance: %.3f%%", 
                                    symbol, TimeframeToString(tf), close_0, ssb_0, direction, dist_ssb_actuelle);
   ...
   if(is_approaching_kijun)
   {
      if(!AlreadyAlerted(symbol, time_buffer[0], tf, "KIJUN")) 
      {
         ...
         string msg = StringFormat("APPROCHE KIJUN | %s | %s | Prix: %.5f | Kijun: %.5f | %s | Distance: %.3f%%", 
                                    symbol, TimeframeToString(tf), close_0, kijun_0, direction, dist_kijun_actuelle);
...
{
   // Vérifier rapidement en essayant de récupérer une donnée
   MqlRates rates[];
...
//+------------------------------------------------------------------+
//| Vérifie si une alerte a déjà été envoyée pour ce symbole,       |
//| ce timeframe, cette bougie et ce type d'alerte                  |
//+------------------------------------------------------------------+
bool AlreadyAlerted(string symbol, datetime bar_time, ENUM_TIMEFRAMES tf, string alert_type)
{
   ...
   for(int i=0; i<size; i++)
   {
      if(symbols_state[i].name == symbol && symbols_state[i].tf == tf)
      {
         ...
         return false; // Symbole+TF trouvé mais pas encore d'alerte pour ce TYPE sur cette bougie
      }
   }
   return false;
}
...
void UpdateAlertState(string symbol, datetime bar_time, ENUM_TIMEFRAMES tf, string alert_type)
{
   ...
   // Ajout d'une nouvelle combinaison symbole+timeframe
